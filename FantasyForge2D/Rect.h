...
public:
	...
	void Draw(Graphics& gfx, const Color& color, unsigned int layer = 0u) const
	{
	...
	void Draw(Graphics& gfx, std::function<Color(unsigned int, unsigned int)> color_func, unsigned int layer = 0u) const
	{
		...
		for (unsigned int y = drawRect.pos.y; y < drawRect.pos.y + drawRect.height; ++y)
		{
			for (unsigned int x = drawRect.pos.x; x < drawRect.pos.x + drawRect.width; ++x)
			{
				gfx.SetPixel(x, y, color_func(x, y), layer);
			}
		}
	}
	void DrawOutline(Graphics& gfx, const Color& color, unsigned int layer = 0u) const
	{
		const Rect<int> gfxRect = gfx.GetRect(layer);
		Rect<int> drawRect = Rect<int>(*this);
		assert(drawRect.IsTouching(gfxRect));
