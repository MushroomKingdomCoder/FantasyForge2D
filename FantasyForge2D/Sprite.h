...
public:
	...
	Sprite(vec2 pos, std::vector<Animation>& animations, unsigned int startingAnimation, vec2 scale = { 1.0f,1.0f }, std::vector<fRect> hit_boxes = {});
	...
	void SetAnimationIndex(unsigned int animation);
	const unsigned int& GetAnimationIndex() const;
	...
	virtual bool Draw(Graphics& gfx, unsigned int layer = 0u) const;
	virtual bool DrawWithTransparency(Graphics& gfx, unsigned int layer = 0u) const;
};
