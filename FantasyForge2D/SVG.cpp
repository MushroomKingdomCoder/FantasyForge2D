...
SVG SVG::GeneratePolygon(unsigned int nSides, vec2 pos, float rot, vec2 scale)
{
	std::vector<std::pair<vec2, vec2>> lineBuffer;
...
SVG SVG::GenerateStar(unsigned int nPoints, float inner_radius_ratio, vec2 pos, float rot, vec2 scale)
{
	std::vector<std::pair<vec2, vec2>> lineBuffer;
