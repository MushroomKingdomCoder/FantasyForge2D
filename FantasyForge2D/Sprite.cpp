...
Sprite::Sprite(vec2 pos, std::vector<Animation>& animations, unsigned int startingAnimation, vec2 scale, std::vector<fRect> hit_boxes)
	:
	position(pos),
	animations(animations),
...
Animation& Sprite::GetAnimation(unsigned int index)
{
	assert(index < animations.size());
	return animations[index];
}

const Animation& Sprite::GetAnimation(unsigned int index) const
{
	assert(index < animations.size());
	return animations[index];
}

void Sprite::SetAnimationIndex(unsigned int animation)
{
	assert(animation < animations.size());
	currentAnimation = animation;
}

const unsigned int& Sprite::GetAnimationIndex() const
{
...
bool Sprite::Draw(Graphics& gfx, unsigned int layer) const
{
	if (imageRect.IsTouching(gfx.GetRect_FLOAT(layer)))
	{
		if (scale != vec2(1.0f, 1.0f))
		{
			animations[currentAnimation].GetCurrentFrame().Draw(gfx, (int)position.x, (int)position.y, (unsigned int)imageRect.GetWidth(), (unsigned int)imageRect.GetHeight(), layer);
		}
		else
		{
...
bool Sprite::DrawWithTransparency(Graphics& gfx, unsigned int layer) const
{
	if (imageRect.IsTouching(gfx.GetRect_FLOAT(layer)))
	{
		if (scale != vec2(1.0f, 1.0f))
		{
			animations[currentAnimation].GetCurrentFrame().DrawWithTransparency(gfx, (int)position.x, (int)position.y, (unsigned int)imageRect.GetWidth(), (unsigned int)imageRect.GetHeight(), layer);
		}
		else
		{
