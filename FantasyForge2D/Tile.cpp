...
bool Tile::Draw(Graphics& gfx, unsigned int layer) const
{
	if (imageRect.IsTouching(gfx.GetRect_FLOAT(layer)))
	{
		if (scale != vec2(1.0f, 1.0f))
		{
			image.GetCurrentFrame().Draw(gfx, (int)position.x, (int)position.y, (unsigned int)GetWidth(), (unsigned int)GetHeight(), layer);
		}
		else
		{
...
bool Tile::DrawWithTransparency(Graphics& gfx, unsigned int layer) const
{
	if (imageRect.IsTouching(gfx.GetRect_FLOAT(layer)))
	{
		if (scale != vec2(1.0f, 1.0f))
		{
			image.GetCurrentFrame().DrawWithTransparency(gfx, (int)position.x, (int)position.y, (unsigned int)GetWidth(), (unsigned int)GetHeight(), layer);
		}
		else
		{
